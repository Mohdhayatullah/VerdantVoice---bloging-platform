...
import { getBlogById, likeBlog, unlikeBlog, followUser, unfollowUser, getFollowersCount, getFollowingCount, getFeedbackByBlog, createFeedback, deleteBlog } from '../api';
...
export default function BlogDetail() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const b = res.data;
        setBlog(b);
        setLiked(!!b.likedByCurrentUser);
        setFollowing(!!b.followingAuthor);
        setLikes(b.likeCount || 0);
        document.title = `${b.title} â€” VerdantVoices`;
        if (b.userId) {
          const [fc, fw] = await Promise.all([getFollowersCount(b.userId), getFollowingCount(b.userId)]);
          setFollowers(fc.data);
          setFollowing2(fw.data);
        }
        loadFeedbacks();
      } catch (e) {
  ...
  const loadFeedbacks = async () => {
    try {
      const r = await getFeedbackByBlog(id);
      setFeedbacks(r.data || []);
    } catch (_) {}
  };
  ...
  const toggleLike = async () => {
    ...
    try {
      const res = liked ? await unlikeBlog(id) : await likeBlog(id);
  ...
  const toggleFollow = async () => {
    ...
    try {
      const res = following ? await unfollowUser(blog.userId) : await followUser(blog.userId);
      setFollowing(!!res.data?.following);
      setFollowers(res.data?.followersCount ?? 0);
      setFollowing2(res.data?.followingCount ?? following2);
      toast.success(res.data?.message || (following ? 'Unfollowed' : 'Following! ðŸŒ¿'));
    } catch (e) { toast.error(e.message); }
  };
  ...
  const submitFeedback = async () => {
    ...
    try {
      ...
      toast.success('Review posted! ðŸŒ¿');
      setComment('');
      setRating(0);
      loadFeedbacks();
    } catch (e) { toast.error(e.message); }
  ...
  if (!blog) return <div className="empty-state"><div className="empty-icon">ðŸ˜•</div><h3>Article not found</h3><Link to="/blogs" className="btn btn-primary" style={{marginTop:20}}>â† Back to Blogs</Link></div>;
      ...
      <div style={{ background:'linear-gradient(160deg,var(--green-900),var(--green-800))', padding:'100px clamp(16px,5vw,60px) 60px', color:'#fff' }}>
        <div style={{ maxWidth:820, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:20, alignItems:'center', fontSize:'0.88rem', color:'var(--green-200)' }}>
            ...
            <span>ðŸ‘ {blog.viewCount||0} views</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:48, maxWidth:1100, margin:'0 auto', padding:'60px clamp(16px,5vw,60px)' }} className="detail-layout">
          ...
          <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'center', padding:'20px 0', borderTop:'1px solid var(--ink-100)', borderBottom:'1px solid var(--ink-100)', marginBottom:36 }}>
            ...
            {isOwner && (
              <div style={{ marginLeft:'auto', display:'flex', gap:10 }}>
                <Link to={`/write/${id}`} className="btn btn-outline btn-sm">âœï¸ Edit</Link>
                <button onClick={handleDelete} className="btn btn-danger btn-sm">ðŸ—‘ Delete</button>
              </div>
            )}
          </div>
          ...
          <div style={{ marginTop:56 }}>
            ...
            {isLoggedIn ? (
              <div style={{ background:'var(--green-50)', border:'1px solid var(--green-200)', borderRadius:'var(--radius-lg)', padding:28, marginBottom:32 }}>
                ...
                <textarea className="form-control" rows="4" placeholder="Write your reviewâ€¦" value={comment} onChange={e=>setComment(e.target.value)} />
                <button className="btn btn-primary" style={{ marginTop:14 }} onClick={submitFeedback} disabled={fbLoading}>
                  {fbLoading ? 'Postingâ€¦' : 'Post Review'}
                </button>
              </div>
            ) : (
        ...
        <aside style={{ position:'sticky', top:90, height:'fit-content' }}>
          <div style={{ background:'#fff', border:'1px solid var(--ink-100)', borderRadius:'var(--radius-lg)', padding:24, marginBottom:24, boxShadow:'var(--shadow-xs)' }}>
            ...
            {[['ðŸ‘ Views', blog.viewCount||0],['â¤ï¸ Likes', likes],['ðŸ“… Published', formatDate(blog.createdAt)]].map(([l,v]) => (
            ...
            <div style={{ display:'flex', justifyContent:'space-between', padding:'10px 0', fontSize:'0.88rem' }}>
              <span style={{ color:'var(--ink-400)' }}>â­ Rating</span>
